...
import com.wb.weibao.databinding.ActivityWaterGageCheckBinding;
 ...
 * 用水-消防栓检测
 */
public class WaterFireCheckActivity extends BaseActivity<BasePresenter, ActivityWaterGageCheckBinding> {

    private List<WaterListModel.DataBean> mDataList = new ArrayList<>();

    @Override
    protected int getLayoutId() {
        return R.layout.activity_water_gage_check;
    }
    ...
    private void getWaterList() {
        Api.getApi().getWaterList(MyApplication.getInstance().getProjectId(), "10002")
        ...
        Api.getApi().getWater(String.valueOf(dataBean.getId()))
                ...
                .subscribe(new BaseNetListener<WaterModel>(this, true) {
                    ...
                    public void onSuccess(WaterModel waterModel) {
                        ...
                        float realtimeData = data.getRealtimeData();
                        mBinding.cvRound.setCompleteDegree(FloatCalculator.divide(realtimeData, 0.02f));
                    }
